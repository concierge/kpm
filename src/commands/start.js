module.exports = moduleCtrl => {
    return {
        run: function(args, api, event) {
            if (args.length !== 1) {
                return api.sendMessage($$`Too many arguments given to start.`, event.thread_id);
            }

            const mod = moduleCtrl.findLoadedModule(args[0], 'integration');
            if (!mod) {
                return api.sendMessage($$`"${args[0]}" is not a valid module/script.`, event.thread_id);
            }

            moduleCtrl.start(mod)
                .then(() => api.sendMessage($$`"${mod.__descriptor.name}" has been started.`, event.thread_id))
                .catch(() => api.sendMessage($$`"${mod.__descriptor.name}" failed to start.`, event.thread_id));
        },
        command: 'start <integrationName>',
        help: $$`Starts an integration`,
        detailedHelp: $$`Starts an integration extended`
    };
};
